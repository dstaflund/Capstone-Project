...
import static com.github.dstaflund.geomemorial.db.GeomemorialDbContract.DATABASE_NAME;
import static com.github.dstaflund.geomemorial.db.GeomemorialDbContract.DATABASE_VERSION;
...
public class GeomemorialDbHelper extends SQLiteOpenHelper {
     ...
     */
    private void applyVersion1Script(@NonNull final SQLiteDatabase db){
     ...
     * @param db to apply script to
     */
    private void applyVersionNScript(@NonNull final SQLiteDatabase db){
     ...
     */
    private void applySqlScript(@NonNull final SQLiteDatabase db, int rawResourceId){
        final InputStream is = mContext.getResources().openRawResource(rawResourceId);
        ...
        try {
            ...
            Log.e(sLogTag, "IO Exception encountered.", e);
        }

        for (final String query : queries.toString().split(sStatementDelimiter)) {
            db.execSQL(query);
        }
    }
}
